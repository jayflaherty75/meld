...
function load (corePtr as Core.Interface ptr) as integer
	if corePtr = NULL then
		print ("**** Map.load: Invalid Core interface pointer")
		return false
	end if
...
function register() as integer
	...
	if _fault = NULL then
		print ("**** Map.register: Missing Fault dependency")
		return false
	end if
	...
	if _core = NULL then
		_fault->throw(_
			...
			"MapLoadingError", "Map module missing Core dependency", _
			__FILE__, __LINE__ _
		)
		return false
	end if
	...
	if _bst = NULL then
		_fault->throw(_
			errors.moduleLoadingError, _
			"MapLoadingError", "Map module missing Bst dependency", _
			__FILE__, __LINE__ _
		)
		return false
	end if
